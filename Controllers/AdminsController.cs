    ...
    public class AdminsController : Controller
    {
        ...
        public ActionResult AssignToRoles(string userId, string roleName)
        {
            ...
            if(user != null)
            {
                ...
                {
                    returnedString.Append(string.Format("{0} was added to {1} roles!", roleName, user.UserName));
                    ViewBag.Message = returnedString.ToString();
                }
                else
                {
                    returnedString.Append(string.Format("{0} was not added to {1} roles!", roleName, user.UserName));
                    ViewBag.Message = returnedString.ToString();
                }
                return View();
            }
